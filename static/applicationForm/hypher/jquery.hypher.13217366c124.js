  ...
  function Hypher(language) {
    ...
    if (language["exceptions"]) {
      ...
      for (; i < exceptions.length; i += 1) {
        this.exceptions[
          exceptions[i].replace(/\u2027/g, "").toLowerCase()
        ] = new RegExp(
    ...
    for (i = 0; i < wordLength; i += 1) {
      ...
      for (j = i; j < wordLength; j += 1) {
        ...
        if (node) {
          ...
          if (nodePoints) {
            for (
              k = 0, nodePointsLength = nodePoints.length;
              k < nodePointsLength;
              k += 1
            ) {
      ...
      return this.each(function () {
        ...
        for (; i < len; i += 1) {
          if (this.childNodes[i].nodeType === 3) {
            this.childNodes[i].nodeValue = window["Hypher"]["languages"][
              language
            ].hyphenateText(this.childNodes[i].nodeValue);
          }
        }
      });
    }
