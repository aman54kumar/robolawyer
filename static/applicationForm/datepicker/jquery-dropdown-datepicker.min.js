  ...
  $.extend(Plugin.prototype, {
    ...
    init: function () {
      ...
      if (this.config.defaultDate) {
        ...
        if (
          !this.config.allowPast &&
          new Date().getTime() > new Date(this.config.defaultDate).getTime()
        ) {
          return;
        }
    ...
    setupMarkup: function () {
      ...
      if (this.element.tagName.toLowerCase() === "input") {
        ...
        hiddenField = this.$element.wrap(
          '<div class="' + this.config.wrapperClass + '"></div>'
        );
        // hiddenField = this.$element.attr('type', 'hidden')
        // .wrap('<div class="' + this.config.wrapperClass + '"></div>');

        var customFieldNameProvided =
            this.config.submitFieldName !== pluginDefaults.submitFieldName,
    ...
    bindChangeEvent: function () {
      ...
      objectRefs.pluginWrapper.on("change", "select", function () {
        ...
        if ($(this).hasClass("year")) {
          ...
          $monthDropdown = $that.addOptionsToDropdown(
            $monthSelect,
            $monthOptions
          );
    ...
    buildDayOptions: function (month, year) {
      ...
      if (
        !this.config.allowPast &&
        year === this.internals.currentYear &&
        month === this.internals.currentMonth &&
      ...
      if (this.config.maxAge != null) {
        if (
          year === this.internals.currentYear - this.config.maxAge &&
          month === this.internals.currentMonth
        ) {
          start1 = this.internals.currentDay;
        }
      ...
      if (
        ...
        new Date(this.config.minDate).getFullYear() === year &&
        new Date(this.config.minDate).getMonth() + 1 === month
      ) {
        start1 =
          start1 < new Date(this.config.minDate).getDate()
            ? new Date(this.config.minDate).getDate()
            : start1;
      }
      ...
      if (this.config.minAge != null) {
        if (
          year === this.internals.currentYear - this.config.minAge &&
          month === this.internals.currentMonth
        ) {
          end2 = this.internals.currentDay;
        }
      ...
      if (
        ...
        new Date(this.config.maxDate).getFullYear() === year &&
        new Date(this.config.maxDate).getMonth() + 1 === month
      ) {
        end2 =
          end2 > new Date(this.config.maxDate).getDate()
            ? new Date(this.config.maxDate).getDate()
            : end2;
      }
    ...
    buildMonthOptions: function (year) {
      ...
      if (
        ...
        start =
          start < new Date(this.config.minDate).getMonth() + 1
            ? new Date(this.config.minDate).getMonth() + 1
            : start;
      }
      if (
        this.config.maxDate !== null &&
        new Date(this.config.maxDate).getFullYear() === year
          ...
          end > new Date(this.config.maxDate).getMonth() + 1
            ? new Date(this.config.maxDate).getMonth() + 1
            : end;
      }
